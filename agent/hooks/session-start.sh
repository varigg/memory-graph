...
if ! curl -sf "$API/health" > /dev/null 2>&1; then
  ...
  uv run python api_server.py >> /tmp/memory-api.log 2>&1 &
  sleep 3
  if ! curl -sf "$API/health" > /dev/null 2>&1; then
...
GOALS_TEXT=$(echo "$GOALS_JSON" | jq -r '
...
PREFS_TEXT=$(curl -sf "$API/preference/active" 2>/dev/null \
  | jq -r 'if length == 0 then "" else "### Active Preferences\n" + (.[] | "- \(.rule)") end' 2>/dev/null \
  || echo "")
