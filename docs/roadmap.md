...
| Typed metadata filtering                       | Implemented initial slice | JSON metadata write support (`metadata` object) and server-side typed metadata filtering across list/search/recall | `README.md`, `docs/phase3-backlog.md`, `tests/test_memory.py`   | `metadata_json` persistence + `metadata_key`/`metadata_value`/`metadata_value_type` read filters      | Consider response-shape normalization if clients need parsed `metadata` instead of raw JSON  |
